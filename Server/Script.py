...
Behavior:
    * Runs for at most 345 minutes per GitHub Actions job.
...
# Stay below GitHub's 6-hour hosted-job limit.
MAX_RUN_SECONDS = 345 * 60  # 5h45m
...
def find_a1_ubuntu_image(
    ...
    """Find the newest standard Ubuntu 24.04/22.04 ARM image.

    We deliberately do NOT pass shape=VM.Standard.A1.Flex to list_images.
    Instead, we discover normal Ubuntu images and select an aarch64 image by
    name. This avoids the overly restrictive image lookup that caused the
    earlier 'no compatible image' result in this tenancy.
    """
    ...
    for image in images:
        display_name = str(getattr(image, "display_name", ""))
        ...
        # Standard Ubuntu, not Minimal Ubuntu.
        if "minimal" in name:
            continue

        # A1/Ampere requires the ARM64/aarch64 build.
    ...
    def score(image: Any) -> tuple[int, str]:
        name = str(getattr(image, "display_name", "")).lower()
...
def main() -> int:
    ...
    while time.monotonic() - start_time < MAX_RUN_SECONDS:
        ...
        try:
            ...
            if exc.status >= 500:
                ...
                print("  The next scheduled workflow can perform a fresh safety check.")
                return 0
    ...
    print("The next scheduled workflow run will check again.")
    print("=" * 78)
    return 0
